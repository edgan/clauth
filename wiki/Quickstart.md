...
| `clauth daemon` | `--status`, `--standby`, `--replace`, `--no-standby`, `--listen`, `--print-token`, `--rotate-token` | run the refresh + auto-switch loop with no TUI |
| `clauth proxy` | `--from`, `--token-file`, `--once`, `--interval`, `--forget` | mirror another machine's accounts onto this one and follow it |
...
- **`daemon --listen`** also serves the REST API over TLS: the status feed, the account switch, and the route `clauth proxy` clones accounts from. It is off unless asked for, and every route needs the bearer token, so anything holding that token can read accounts from that daemon. `--print-token` and `--rotate-token` manage it, and work whether or not a daemon is running. Full detail in [Daemon](Daemon).
- **`proxy`** makes this host a read-only follower of another one. Switching, login, delete and `clauth daemon` are refused here, and the usage refresher never starts, because the origin owns polling and rotation. The refresh token never crosses the wire, so a replica can spend an account but never rotate its chain. `clauth proxy --forget` re-arms the host. Full detail in [Proxy](Proxy).

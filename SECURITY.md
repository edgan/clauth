...
`~/.clauth/auth_token.json` and printed by `clauth daemon --print-token`. It exposes three
operations: reading the status feed, switching the active account, and cloning the accounts
for a replica (see below). The feed itself carries what `status.json` carries: names, tiers,
percentages, timestamps, never a token or key. Connections persist and may be pipelined; `Content-Length` is the only

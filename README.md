...
- 📊 **Monitor** live 5h / 7d rate-limit bars, a global token dashboard with API-equivalent cost, plus a live Claude status-incident feed — and wire `clauth statusline` into your status line to read the exact percentage Claude Code shows, instead of the rounded one `/usage` returns, from a session on this machine or on any other (`clauth statusline --to <host>`)

...
// ── TLS fixture ───────────────────────────────────────────────────────────────
//
// Shared by the REST listener's own tests and by `clauth proxy`'s, which drive a
// real client against a real listener. It lived inside `daemon_api_server.rs`
// while that was its only consumer; a second one is what puts it here.

...
const BASH_TEMPLATE: &str = r#"_clauth() {
    ...
    if [ "$COMP_CWORD" -eq 1 ]; then
        ...
        COMPREPLY=( $(compgen -W "${profiles} start login capture delete disable enable rolling-token static-token which statusline list jobs sessions resume info daemon proxy status mcp herdr completions --theme" -- "${cur}") )

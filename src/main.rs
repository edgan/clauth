...
fn dispatch(cli: Cli) -> Result<()> {
    ...
    match command {
        ...
        Command::Capture { profile } => {
            // A capture writes a login into a profile, which on a replica is
            // the origin's to do -- refused like `login` and for the same reason.
            proxy::refuse_if_replica("capture the login")?;
            cmd_capture(&profile)
        }
        Command::Delete {
            profile,
        ...
        Command::Statusline(a) => statusline::run(&a),
